...
func TestCreateTestJob(t *testing.T) {
	...
	v1.Add(&job.RunParam{
		Required:  true,
		Name:      "cluster_id",
		ParamDesc: "job运行时的k8s集群",
		Value:     "k8s-test",
	})
	v1.Add(&job.RunParam{
		Required:  true,
		Name:      "namespace",
		ParamDesc: "job运行时的namespace",
		Value:     "default",
	})
...
func TestUpdateBuildJob(t *testing.T) {
	...
	v1.Add(&job.RunParam{
		Required:  false,
		Name:      "APP_DOCKERFILE",
		ParamDesc: "应用git代码仓库中用于构建镜像的Dockerfile路径",
		Value:     "Dockerfile",
		Example:   "Dockerfile",
	})
	v1.Add(&job.RunParam{
	...
	v1.Add(&job.RunParam{
		...
		Name:      "GIT_SSH_SECRET",
		ParamDesc: "用于拉取git仓库代码的secret名称, kubectl create secret generic git-ssh-key --from-file=id_rsa=${HOME}/.ssh/id_rsa",
		Example:   "git-ssh-key",
		Value:     "git-ssh-key",
	})
	...
	v1.Add(&job.RunParam{
		...
		ParamDesc: "用于推送镜像的secret名称, 具体文档参考: https://github.com/GoogleContainerTools/kaniko#pushing-to-docker-hub",
