...
2. 添加配置文件(默认读取位置: etc/mpaas.toml)
```sh
$ 编辑样例配置文件 etc/mpaas.toml.book
$ mv etc/mpaas.toml.book etc/mpaas.toml
```
...
```sh
docker buildx build  --platform linux/amd64 -t mpaas .
```
